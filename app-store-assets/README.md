# Apple App Store Publishing Guide

## Prerequisites

### Apple Developer Account

- **Cost**: $99/year
- **Required**: For App Store publishing
- **Sign up**: <https://developer.apple.com/programs/>

## Required Assets

### App Icons

- **App Icon**: 1024 x 1024 px (PNG, no transparency)
- **App Store Screenshots**:
  - iPhone 6.7" (1290 x 2796 px)
  - iPhone 6.5" (1242 x 2688 px)
  - iPhone 5.5" (1242 x 2208 px)
  - iPad Pro 12.9" (2048 x 2732 px)
  - iPad Pro 11" (1668 x 2388 px)

### App Information

- **App Name**: "Erev Shabbos Weather Report"
- **Subtitle**: 30 characters max
- **Keywords**: 100 characters max
- **Category**: Weather
- **Content Rating**: 4+

## App Store Listing Content

### App Name

"Erev Shabbos Weather Report"

### Subtitle (30 chars max)

"Weather & Candle Times for Shabbos"

### Keywords (100 chars max)

Shabbos,weather,candle,lighting,Jewish,forecast,Shabbat,Hebrew,parshah,weekend

### App Description

```
Get accurate weather forecasts and candle lighting times for Shabbos preparation. Plan your weekend with detailed hourly weather updates, printable PDF reports, and location-based Shabbos information. Perfect for families preparing for a peaceful and organized Shabbos.

🕯️ Candle Lighting Times
• Accurate candle lighting times for your location
• Hebrew date and parshah information
• Havdalah times included

🌤️ Weather Forecasts
• Detailed weather forecasts for Friday and Saturday
• Hourly weather updates
• Temperature, precipitation, and wind information
• Beautiful weather icons and descriptions

📍 Location-Based
• Uses your current location for accurate times
• Automatic timezone detection
• Works worldwide

📱 User-Friendly Features
• Clean, intuitive interface
• Printable PDF weather reports
• Offline access to candle times
• No ads or in-app purchases

Perfect for:
• Jewish families preparing for Shabbos
• Anyone needing weather information for weekend planning
• Communities organizing Shabbos events
• Travelers needing local weather and timing information

Download now and make your Shabbos preparation easier and more organized!
```

### What's New in This Version

```
Initial release of Erev Shabbos Weather Report!

Features:
• Accurate candle lighting times for your location
• Detailed weather forecasts for Friday and Saturday
• Hebrew date and parshah information
• Printable PDF weather reports
• Clean, intuitive interface
• No ads or in-app purchases
```

### App Store Categories

- **Primary**: Weather
- **Secondary**: Lifestyle

### Content Rating

- **Violence**: None
- **Sex**: None
- **Language**: None
- **Controlled Substances**: None
- **User Generated Content**: None

## App Store Connect Setup

### 1. Create App

- Go to App Store Connect
- Click "My Apps" → "+" → "New App"
- Fill in basic information

### 2. App Information

- **Bundle ID**: com.shabbospdf.weather
- **SKU**: shabbospdf-weather-ios
- **User Access**: Full Access

### 3. Pricing and Availability

- **Price**: Free
- **Availability**: All countries
- **Release Type**: Manual

### 4. App Review Information

- **Contact Information**: Your email
- **Demo Account**: Not required
- **Notes**: "This app provides weather forecasts and candle lighting times for Jewish Shabbos preparation. It uses location services to provide accurate local information."

## Privacy Requirements

### Privacy Policy

- Required for all apps
- Must be accessible via URL
- Should cover data collection and usage

### App Privacy Details

- **Data Used to Track You**: None
- **Data Linked to You**: None
- **Data Not Linked to You**: Location (for weather/times)

## Publishing Steps

1. **Get Apple Developer Account** ($99/year)
2. **Create App in App Store Connect**
3. **Upload iOS build** (via EAS or Xcode)
4. **Fill in App Store listing**
5. **Upload screenshots and app icon**
6. **Set up privacy policy**
7. **Submit for Review** (1-7 days typically)

## Notes

- iOS apps typically review faster than Android
- Apple is strict about privacy and app quality
- Ensure all permissions are properly explained
- Test thoroughly on multiple iOS devices
- Consider creating an App Store preview video

## Common Rejection Reasons

- Missing privacy policy
- Incomplete app information
- Poor app icon quality
- Missing screenshots
- App crashes during review
- Unclear app purpose
